...
class HiveServer2Operator(BaseOperator):
    """
    Executes hql code in a specific Hive database using Hiveserver2.

    :param hql: the hql to be executed
    :type hql: string
    :param hiveserver2_conn_id: reference to the Hive database
    ...
    def get_hook(self):
        return HiveServer2Hook(
            hiveserver2_conn_id=self.hiveserver2_conn_id,
            run_as=self.run_as,
            mapred_queue=self.mapred_queue,
            mapred_queue_priority=self.mapred_queue_priority,
            mapred_job_name=self.mapred_job_name)
    ...
    def execute(self, context):
        logging.info('Executing: ' + self.hql)
        self.hook = self.get_hook()
        self.hook.run(hql=self.hql, schema=self.schema,
                      hive_conf=context_to_airflow_vars(context))
